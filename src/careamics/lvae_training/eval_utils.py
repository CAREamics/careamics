"""LVAE evaluation utilities.

DEPRECATED: This module is kept only for backwards compatibility with the ND2
MicroSplit training script (``scripts/microsplit_nd2_train.py``), which drives
prediction directly against ``LCMultiChDloader``-style datasets that live in
``careamics.lvae_training.dataset``. New code should use the Lightning
``predict`` flow via ``CareamicsDataModule`` + ``MicroSplitModule``, then stitch
with ``careamics.prediction_utils.convert_outputs_microsplit``. This module will
be removed once the ND2 script is ported to the NG datamodule.
"""
...
warnings.warn(
    "careamics.lvae_training.eval_utils is deprecated and kept only for the "
    "ND2 MicroSplit script; prefer the Lightning predict flow via "
    "CareamicsDataModule + MicroSplitModule + convert_outputs_microsplit.",
    DeprecationWarning,
    stacklevel=2,
)
...
def get_predictions(
    ...
    model : pytorch_lightning.LightningModule
        Lightning model used for prediction (any module exposing
        ``model(inp) -> (rec, _)`` and ``model.model.predict_logvar``).
    dset : Dataset
...
def get_single_file_predictions(
    ...
    with torch.no_grad():
        for batch in tqdm(dloader, desc="Predicting tiles"):
            ...
            # logvar_arr is never read; accumulating it per MMSE sample costs
            # n_batches * mmse_count * batch tensors of host RAM (~80 GB at batch 64,
            # mmse 50 on a 20-frame eval) and OOM-killed the job. Dropped.
            del logvar
