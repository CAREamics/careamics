...
class GridIndexManagerRef:
    ...
    def __post_init__(self):
        assert (
            set([len(ds) for ds in self.data_shapes[0]]).pop()
            == set([len(ds) for ds in self.data_shapes[1]]).pop()
        ), "Data shape for all channels must be the same" # TODO better way tu assert this
        assert set([len(ds) for ds in self.data_shapes[0]]).pop() == len(
            self.grid_shape
    ...
    def get_individual_dim_grid_count(self, dim: int):
        ...
        if self.grid_shape[dim] == 1 and self.patch_shape[dim] == 1:
            return self.data_shapes[dim]
        elif self.tiling_mode == TilingMode.PadBoundary:
            return int(np.ceil(self.data_shapes[dim] / self.grid_shape[dim]))
        elif self.tiling_mode == TilingMode.ShiftBoundary:
            excess_size = self.patch_shape[dim] - self.grid_shape[dim]
            return int(
                np.ceil((self.data_shapes[dim] - excess_size) / self.grid_shape[dim])
            )
        else:
            excess_size = self.patch_shape[dim] - self.grid_shape[dim]
            return int(
                np.floor((self.data_shapes[dim] - excess_size) / self.grid_shape[dim])
            )
    ...
    def get_grid_index(self, dim: int, coordinate: int):
        ...
        assert dim >= 0, "Dimension must be greater than or equal to 0"
        assert (
            coordinate < self.data_shapes[dim]
        ), f"Coordinate {coordinate} is out of bounds for data shape {self.data_shapes}"
    ...
    def dataset_idx_from_grid_idx(self, grid_idx: tuple):
        ...
        assert len(grid_idx) == len(
            self.data_shape
        ), f"Dimension indices {grid_idx} must have the same dimension as data shape {self.data_shapes}"
        index = 0
        for dim in range(len(grid_idx)):
            index += grid_idx[dim] * self.grid_count(dim)
        return index
    ...
    def get_location_from_dataset_idx(self, dataset_idx: int):
        ...
        """
        grid_idx = []
        for dim in range(len(self.data_shapes)):
            grid_idx.append(dataset_idx // self.grid_count(dim))
            dataset_idx = dataset_idx % self.grid_count(dim)
        location = [
            self.get_gridstart_location_from_dim_index(dim, grid_idx[dim])
