...
def select_write_func(
    ...
    """
    Return a function to write images. If `write_type` is "custom" then `write_func`,
    otherwise the known write function is selected.
...
def select_write_extension(
    ...
    """
    Return an extension to add to file paths. If `write_type` is "custom" then 
    `write_extension`, otherwise the known write extension is selected.
