...
def test_setting_image_means_std():
    ...
    model = NormalizeConfig(image_means=[0.5], image_stds=[0.5])
...
def test_error_different_length_means_stds():
    ...
    with pytest.raises(ValueError):
        NormalizeConfig(image_means=[0.5], image_stds=[0.5, 0.6])

    with pytest.raises(ValueError):
        NormalizeConfig(image_means=[0.5, 0.6], image_stds=[0.5])
