...
@pytest.mark.mps_gh_fail
def test_smoke_n2v_zarr_preserves_chunks(tmp_path):
    shape = (16, 32, 32)
    axes = "ZYX"
    chunks = (8, 16, 16)
    ...
    # write train array to zarr with explicit chunks
    train_dir = tmp_path / "train"
    train_dir.mkdir()
    file_name = "image.zarr"
    g = zarr.open_group(train_dir / file_name, mode="w")
    arr = g.create_array("array", data=train_array, chunks=chunks)
    ...
    dataset_cfg = create_ng_data_configuration(
        data_type="array", axes=axes, patch_size=(8, 16, 16), batch_size=2
    )
    model = N2VModule(algorithm_config=algorithm_config)
    data = CareamicsDataModule(
        data_config=dataset_cfg, train_data=train_array, val_data=val_array
    )
    dirpath = tmp_path / "predictions"
    predict_writer = PredictionWriterCallback(dirpath=dirpath)
    trainer = Trainer(
        ...
        callbacks=[
            ModelCheckpoint(
                dirpath=tmp_path / "checkpoints", filename="test_lightning_api"
            ),
            predict_writer,
        ],
    )
    ...
    output_file = dirpath / f"{file_name[:-len('.zarr')]}_output.zarr"
    save_data = zarr.open_array(output_file, path="array")
    assert tuple(save_data.chunks) == chunks
