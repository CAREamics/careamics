...
@pytest.fixture
def prediction_writer_callback(write_strategy: WriteStrategy, dirpath: Union[Path, str]):
...
def test_init_dirpath_relative_path(prediction_writer_callback):
    ...
    with patch("pathlib.Path.cwd", return_value=Path("/current/working/dir")):
        ...
        assert prediction_writer_callback.dirpath == Path("/current/working/dir/relative/path")
