...
1. `original_axes` is implemented as an instance attribute.
2. `original_data_shape` is implemented as an instance attribute.
3. `data_shape` is implemented as an instance attribute.
4. We decided to make the source the file path followed by a `#` followed by the internal dataset path, e.g. `/data/hdf5_dataset.h5#/image_0`.
5. HDF5 data can be sliced just like NumPy arrays.